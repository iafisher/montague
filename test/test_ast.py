...
class FormulaToStrTest(unittest.TestCase):
    ...
    def test_if_and_only_if_to_str(self):
        self.assertEqual(str(IfAndOnlyIf(Var('a'), Var('b'))), 'a <-> b')
    ...
    def test_lambda_to_str(self):
        ...
        self.assertEqual(
            str(And(Lambda('x', Var('x')), Lambda('y', Var('y')))), '[λx.x] & [λy.y]'
        )
    ...
    def test_not_to_str(self):
        ...
        self.assertEqual(str(Not(Or(Var('x'), Var('y')))), '~[x | y]')

    def test_binary_operators_to_str(self):
        self.assertEqual(str(And(Or(Var('a'), Var('b')), Var('c'))), '[a | b] & c')
        self.assertEqual(str(Or(And(Var('a'), Var('b')), Var('c'))), 'a & b | c')
        self.assertEqual(str(Or(Var('a'), Or(Var('b'), Var('c')))), 'a | b | c')
        self.assertEqual(str(And(Var('a'), And(Var('b'), Var('c')))), 'a & b & c')

    def test_nested_exists_and_for_all_to_str(self):
        self.assertEqual(
            str(And(ForAll('x', Var('x')), Exists('x', Var('x')))), '[∀ x.x] & [∃ x.x]'
        )
...
class TypeToStrTest(unittest.TestCase):
    ...
    def test_recursive_type_to_str(self):
        self.assertEqual(str(ComplexType(TYPE_ENTITY, TYPE_TRUTH_VALUE)), '<e, t>')
...
class ReplacerTest(unittest.TestCase):
    def test_simple_replace_variable(self):
        self.assertTupleEqual(Var('x').replace_variable('x', Var('y')), Var('y'))
    ...
    def test_replace_predicate(self):
        ...
        self.assertTupleEqual(
            tree.replace_variable('P', Var('Good')), Call(Var('Good'), Var('x'))
        )
    ...
    def test_replace_variable_in_quantifiers(self):
        tree = ForAll(
            'x',
            Or(
                And(ForAll('b', Var('b')), Exists('b', Var('b'))), Exists('y', Var('b'))
            ),
        )
        self.assertTupleEqual(
            ...
            ForAll(
                'x',
                Or(
                    And(ForAll('b', Var('b')), Exists('b', Var('b'))),
                    Exists('y', Var('bbb')),
                ),
            ),
        )
    ...
    def test_recursive_replace_variable(self):
        ...
        self.assertTupleEqual(
            ...
            Call(
                Call(Call(Var('BFP'), Var('j')), Lambda('x', Var('x'))),
                And(Var('j'), Var('y')),
            ),
        )
    ...
    def test_replace_variable_in_iota(self):
        ...
        self.assertTupleEqual(
            tree.replace_variable('y', Var('b')), Iota('x', And(Var('x'), Var('b')))
        )
